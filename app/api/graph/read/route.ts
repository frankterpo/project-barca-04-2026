...
/**
 * POST /api/graph/read
 * Body: { "query": "holdings_for_run", "params": { "run_id": "demo-v1" }, "branch": "main" }
 */
  ...
  try {
    ...
    if (!queryName) {
      return NextResponse.json(
        { ok: false, error: { code: "INVALID_BODY", message: '"query" (named query) is required' } },
        { status: 400 },
      );
    }
    ...
    const data = await client.read(queryName, params, { branch });
